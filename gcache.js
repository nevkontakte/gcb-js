...
$(function(){new Gcb.Navigator('body');new Gcb.UserVoice();});
...
-(this.foreignAlert.is(':visible')?this.foreignAlert.outerHeight(true):0));},replaceContent:function(data){var doc=this.view.get(0).contentDocument;if(doc==null){this.view.attr('src','about:blank');setTimeout($.proxy(function(){this.replaceContent(data)},this),0);return;}
doc.open();doc.write(data);doc.close();$(doc).find('div:first').css('margin','0').css('padding','0');this.watchLinks(1,0);},watchLinks:function(delay,hookedLinks){var doc=$(this.view.get(0).contentDocument);var links=doc.find('a');var linksLength=links.length;links.filter(":not(.--gcb-hooked-link)").on("click",function(){can.route.attr('url',this.href);return false;}).addClass("--gcb-hooked-link");if(doc.readyState!=="complete"){if(hookedLinks==linksLength){delay=Math.min(delay*2,300);}else{delay=1;}
setTimeout($.proxy(function(){this.watchLinks(delay,linksLength);},this),delay);}},'{window} resize':function(){this.resize();},'go/:url route':function(data){var doc=this.view.get(0).contentDocument;if(data['url']==""){doc.location.replace(this.initial);}
...
$.ajax(query,{"context":this,"success":this.replaceContent,"dataType":"html"});}});publish.Navigator=can.Control({init:function(el){$('#greetings').remove();$('body *').hide();var meta=[{'name':'viewport','content':'width=device-width, initial-scale=1.0'}];var head=$('head');for(var i=0;i<meta.length;i++){var tag=$('<meta>');tag.attr(meta[i]);head.append(tag);}
...
//@ sourceMappingURL=assets/gcache.map.js